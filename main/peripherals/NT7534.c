...
static void set_reset(uint8_t res);
static void set_data_command(uint8_t dc);
static void disp_spi_send_data(uint8_t *data, unsigned int length);
...
static void disp_spi_send_data(uint8_t *data, unsigned int length) {
    ...
    int i=0;
    for (i=0;i<length;i++) {
        spi1_master_exchange(data[i], SPI_SPEED_FAST);
    }
