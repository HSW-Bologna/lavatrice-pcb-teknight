...
//        }
        ...
        if (model_get_status_step(p)==STATO_STEP_RAF)
        {
            ...
            else // INV prevista
            {
                if ((stopwatch_get_state(&ct_moto_cesto) == TIMER_STOPPED) || stopwatch_is_timer_reached(&ct_moto_cesto, timestamp)) 
                {
                    ...
                    if (p->status.f_ava_ind==0) // avvio prima volta
                    {
        ...
        //            ct_anti_piega_max = p->pmac.tempo_max_antipiega*60;          
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_NO_GIRO) /* INF NO INV -------------------------- */
            {
                if  (stopwatch_is_timer_reached(&ct_anti_piega, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega);
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_NO_PAUSA) /* INF NO INV -------------------------- */
            {
                if (stopwatch_is_timer_reached(&ct_anti_piega_pausa, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega_pausa);
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_SI_GIRO_ORA) /* INF SI INV PAUSA -------------------- */
            {
                if  (stopwatch_is_timer_reached(&ct_anti_piega, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega);
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_SI_PAUSA_ORA) /* INF SI INV MOTO INV---------------------------- */
            {
                if (stopwatch_is_timer_reached(&ct_anti_piega_pausa, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega_pausa);
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_SI_GIRO_ANT) /* INF SI INV MOTO INV ----------------- */
            {
                if  (stopwatch_is_timer_reached(&ct_anti_piega, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega);
            ...
            if (p->status.nf_anti_piega==ANTIPIEGA_DURATA_INFINITA_INV_SI_PAUSA_ANT) /* INF SI INV -------------------------- */
            {
                if (stopwatch_is_timer_reached(&ct_anti_piega_pausa, get_millis()))
                {
                    stopwatch_init(&ct_anti_piega_pausa);
