...
typedef int(*stato_guard_t)(model_t *);

static int stato_off_event_manager(model_t *pmodel, stato_event_code_t event);
static int stato_off_guard(model_t *pmodel);
static int stato_on_event_manager(model_t *pmodel, stato_event_code_t event);
static int stato_on_guard(model_t *pmodel);

static const struct {
    stato_guard_t guard;
    stato_event_manager_t event;
} event_managers[] = {
    {stato_off_guard, stato_off_event_manager},
    {stato_on_guard, stato_on_event_manager},
};
