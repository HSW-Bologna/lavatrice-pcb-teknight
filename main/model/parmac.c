...
#define FINT(i) ((parameter_user_data_t){parmac_descriptions[i], formatta, NULL, NULL})
    ...
    parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.logo,                            NULL,   NULL,   0,   5,    0,    1,    AL_USER,     FINT(PARMAC_DESCRIPTIONS_LOGO),                 NULL,       NULL);
    parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.livello_accesso,                 NULL,   NULL,   0,   2,    0,    1,    AL_USER,     FINT(PARMAC_DESCRIPTIONS_LIVELLO_ACCESSO),      NULL,       NULL);
    // clang-format on
...
void parmac_operation(size_t parameter, int op) {
    parameter_operator(parameter_get_handle(parameters, NUM_PARAMETERS, parameter, AL_USER ), op);
}
...
const char *parmac_get_description(const model_t *pmodel, size_t parameter) {
    ...
    parameter_user_data_t data =
        parameter_get_user_data(parameter_get_handle(parameters, NUM_PARAMETERS, parameter, AL_USER ));
...
void parmac_format_value(char *string, size_t parameter) {
    parameter_handle_t *  par  = parameter_get_handle(parameters, NUM_PARAMETERS, parameter, AL_USER );
...
size_t parmac_get_tot_parameters(void) {
    return parameter_get_count(parameters, NUM_PARAMETERS, AL_USER );
}
