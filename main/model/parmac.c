...
static const char *languages[] = {"LINGUA", "LANGUAGE", "LANGUE", "SPRACHE", "JEZYK"};
...
void parmac_setup_commissioning(model_t *p) {
    parameters[PARMAC_COMMISSIONING_LINGUA] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.lingua, NULL, NULL, 0, 4, 0,
...
void parmac_setup_full(model_t *p, size_t chunk, int reset) {
    ...
    switch (chunk) {
        ...
        case 0: // 01
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.lingua,                              NULL,   NULL,   0,      4,      0,      1,      AL_USER,     FINT(PARMAC_DESCRIPTIONS_LINGUA),                                  NULL,       NULL);
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.lingua_max,                          NULL,   NULL,   1,      5,      4,      1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_LINGUA_MAX),                              NULL,       NULL);
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.logo_ditta,                          NULL,   NULL,   0,      5,      1,      1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_STRINGA_DITTA_VISUALIZZATA),              NULL,       NULL);
            ...
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.allarme_inverter_na_nc,              NULL,   NULL,   0,      1,      0,      1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_ALLARME_INVERTER_NA_NC),                  NULL,       NULL);
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.allarme_filtro_na_nc,                NULL,   NULL,   0,      1,      1,      1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_ALLARME_FILTRO_NA_NC),                    NULL,       NULL);
...
//          parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.temperatura_sicurezza_1,             NULL,   NULL,   1,      145,    135,    1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_TEMPERATURA_SICUREZZA_1),                 NULL,       NULL);
            parameters[i++] = PARAMETER_C99(PARAMETER_TYPE_UINT8, &p->pmac.temperatura_sicurezza_1,             NULL,   NULL,   1,      135,    130,    1,      AL_TECH,     FINT(PARMAC_DESCRIPTIONS_TEMPERATURA_SICUREZZA_1),                 NULL,       NULL);
