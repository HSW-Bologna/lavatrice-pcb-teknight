...
static void open_page(model_t *model, void *data) {
    view_common_title(lv_scr_act(), "VELOCITA' 2");
...
static view_message_t process_page_event(model_t *model, void *arg, pman_event_t event) {
    ...
    switch (event.code) {
        case VIEW_EVENT_KEYPAD: {
            ...
            if ((event.key_event.event == KEY_PRESSING || event.key_event.event == KEY_LONGPRESS) && event.key_event.code == BUTTON_PLUS) {
                if (page_data.speed==100) {
                    page_data.speed=0;
                }
                page_data.speed++;
            ...
            if ((event.key_event.event == KEY_PRESSING || event.key_event.event == KEY_LONGPRESS) && event.key_event.code == BUTTON_MINUS) {
                if (page_data.speed==0) {
                    page_data.speed=100;
                }
                page_data.speed--;
...
const pman_page_t page_pwm2_test = {
    .create        = create_page,
    .open          = open_page,
    .update        = update_page,
